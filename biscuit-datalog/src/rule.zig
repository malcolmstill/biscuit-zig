...
pub const Rule = struct {
    ...
    pub fn remapSymbols(rule: Rule, old_symbols: *const SymbolTable, new_symbols: *SymbolTable) !Rule {
        ...
        for (body.items, 0..) |predicate, i| {
            body.items[i] = try predicate.remapSymbols(old_symbols, new_symbols);
        }

        for (expressions.items, 0..) |expression, i| {
            expressions.items[i] = try expression.remapSymbols(old_symbols, new_symbols);
        }

        for (scopes.items, 0..) |scope, i| {
            scopes.items[i] = try scope.remapSymbols(old_symbols, new_symbols);
        }

        return .{
            .head = try rule.head.remapSymbols(old_symbols, new_symbols),
            .body = body,
